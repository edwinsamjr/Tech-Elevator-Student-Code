...
public class Exercises {
    ...
    public int max1020(int a, int b) {
        ...
        boolean isBGreaterOrEqualToA = b >= 1;

        if ((isAInRange && isAGreaterOrEqualToB) || (isAInRange && !isBInRange)) {
            return a;
        } else if ((isBInRange && isBGreaterOrEqualToA) || (isBInRange && !isAInRange)) {
            return b;
        }
        return 0;
    ...
    public int dateFashion(int you, int date) {
        boolean isYouVeryStylish = you >= 8;
        boolean isDateVeryStylish = date >= 8;
        boolean isYouUnstylish = you <= 2;
        boolean isDateUnstylish = date <= 2;

        if (isDateUnstylish || isYouUnstylish) {
            return 0;
        } else if (isDateVeryStylish || isYouVeryStylish) {
            return 2;
        }
    ...
    public String yourCakeAndEatItToo(double mealAmount, boolean isBirthday) {
        boolean standardAmount = mealAmount <= 10.0;
        boolean specialAmount = mealAmount > 10.0 && mealAmount <= 15.0;
        boolean ginormousAmount = mealAmount > 15.0;

        if (mealAmount > 15) {
            return "ginormous";
        } else if (specialAmount && !isBirthday) {
            return "special";
        } else if (specialAmount && isBirthday) {
            return "ginormous";
        } else if (standardAmount && !isBirthday) {
            return "standard";
        } else if (standardAmount && isBirthday && mealAmount >= 5.0) {
            return "special";
        } else if (standardAmount && isBirthday && mealAmount < 5) {
            return "standard";
        }
    ...
    public boolean in1To10(int n, boolean outsideMode) {
        ...
        boolean isOutRange = n <= 1 || n >= 10;

        if (isInRange && !outsideMode) {
            return true;
        } else if (isOutRange && outsideMode) {
            return true;
        }
        return false;
    }
    ...
    public int teaParty(int tea, int candy) {
        boolean isTeaGreaterThanFive = tea >= 5;
        boolean isCandyGreaterThanFive = candy >= 5;
        boolean isTeaDoubleCandy = tea >= (2 * candy);
        boolean isCandyDoubleTea = candy >= (2 * tea);

        if (!isTeaGreaterThanFive || !isCandyGreaterThanFive) {
            return 0;
        } else if (isCandyDoubleTea || isTeaDoubleCandy) {
            return 2;
        }
    ...
    public int luckySum(int a, int b, int c) {
        boolean isAThirteen = a == 13;
        boolean isBThirteen = b == 13;
        boolean isCThirteen = c == 13;

        if ((isAThirteen && isBThirteen) || (isAThirteen && isCThirteen) ){
            return 0;
        } else if (isBThirteen){
            return a;
        } else if (isCThirteen){
